...
test("timer follows the fixed sound and no-vibration policy",()=>{assert.match(source,/state\.soundType = "external-sport"/);assert.doesNotMatch(source,/id="soundType"|navigator\.vibrate/);assert.match(source,/standaloneLeft>=1&&standaloneLeft<=4/);assert.match(source,/activeLeft>=1&&activeLeft<=4/);assert.match(source,/de siste fire sekundene/);});
