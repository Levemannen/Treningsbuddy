const tabs = [["home","Start"],["bank","Øvelser"],["history","Historikk"]];
    ...
    core:{"Planke-varianter":["Planke","Høy planke","Lang planke","RKC-planke","Planke med benløft","Planke med armstrekk","Planke med skuldertouch","Planke til albue (commando plank)","Sideplanke","Sideplanke med benløft","Sideplanke med rotasjon","Copenhagen Plank","Omvendt planke","Planke/Rollout"],"Dead bug / Bird dog":["Dead Bug","Dead bug med armstrekk","Dead bug med benstrekk","Dead bug med arm og ben","Bird Dog","Bird dog med pause","Bird dog crunch"],"Hollow body":["Hollow Hold","Hollow rocks","Hollow tuck hold","Hollow body flutter kicks","Hollow body scissors"],"Crunch-varianter":["Crunch","Reverse Crunch","Bicycle crunch","Toe touch crunch","Heel touch crunch","Oblique crunch","Cross-body crunch","Double crunch"],"Situp-varianter":["Situps","Butterfly situp","Jackknife situp","V-up","Tuck-up","Sprinter situp","Boat Pose"],"Beinhev-varianter":["Leg Raises","Liggende beinhev","Enkeltbeinshev","Saksespark","Flutter kicks","Leg raise hold","Reverse leg raise","Ab Wheel Rollout"],"Skrå magemuskler":["Russian Twist","Russian twist (uten vekt)","Side crunch","Sideplanke med hoftefall","Windshield wipers","Oblique V-up","Plank Knee To Elbow","Pallof Press","Wood Chop"],"L-sit-familien":["Tuck sit","L-sit","Enkeltbeins L-sit","V-sit","Tuck hold"],"Rygg og bakside core":["Superman","Alternating superman","Superman hold","Back Extension","Back extension på gulv","Svømmeren (Swimmer)","Glute Bridge","Enbeins glute bridge","Hip thrust (kroppsvekt)","Hip Extension Hold"],"Dynamiske core-øvelser":["Mountain Climbers","Cross-body mountain climbers","Bear Crawl","Crab walk","Leopard crawl","Army crawl","Inchworm","Plank Walkouts","Plank jacks","Commandos","Shoulder Taps","Stir The Pot"],"Avanserte kroppsvektøvelser":["Dragon flag","Hanging knee raise","Hanging leg raise","Hanging windshield wipers","Toes to bar","Front lever tuck","Front lever raises"]},
      ...
      if(!["home","bank","history","builder","detail","favworkouts"].includes(state.tab))state.tab="home";
      ...
      function beginWorkout(name,cat,ex,timerChoice=state.timerChoice||"none") { state.builderName=name;state.builder=ex.map(e=>({...e}));state.builderCat=cat;state.builderPickCat=cat;state.timerChoice=cat==="tabata"?"tabata":hasIntervalTimer(cat)?"interval":timerChoice;state.tab="builder";if(hasIntervalTimer(cat)){state.builderStep="timer";state.builderMode="build";save();render();requestAnimationFrame(()=>document.querySelector(".commonTimerSettings")?.scrollIntoView({block:"start"}));return;}start(name,cat,state.builder); }
      ...
      function renderFavoriteWorkouts() {
        ...
        if(!state.favoriteCategory){
          view.innerHTML=`<div class="section"><button class="secondary" id="backHome">Tilbake</button><h2>Favorittøkt</h2></div><div class="flowGrid">${flowOptions.map(([id,label])=>{const count=allPrograms.filter(p=>p.cat===id).length;return `<button class="flowChoice panel" data-favorite-cat="${id}"><span>${label}</span><small>${count} ${count===1?"lagret økt":"lagrede økter"}</small></button>`;}).join("")}</div>`;
      ...
      const commonTimerSettingsHtml = cat => {
        if(!hasIntervalTimer(cat))return "";
      ...
      const syntheticSound = (kind="countdown",remaining=1) => {
        ...
        const vol=volumeLevel(),now=timerAudioContext.currentTime,master=timerAudioContext.createGain(),filter=timerAudioContext.createBiquadFilter(),delay=timerAudioContext.createDelay(),echo=timerAudioContext.createGain();
        filter.type="lowpass";filter.frequency.setValueAtTime(760,now);delay.delayTime.setValueAtTime(.1,now);echo.gain.setValueAtTime(.07,now);
        ...
        const base=kind==="rest"?185:kind==="complete"?247:kind==="warning"?174:kind==="countdown"?(remaining===1?294:262):220;
        const tones=kind==="countdown"?[[base,"triangle",0,.34],[base*1.5,"sine",.02,.28]]:[[base,"triangle",0,length],[base*1.5,"sine",.025,length*.86],[base*2,"triangle",.055,length*.68]];
      ...
      function builderLine(e,i) {
        ...
        if (nextTimer) nextTimer.onclick=()=>{if(!state.builder.length)return;if(hasIntervalTimer(state.builderCat)){state.builderStep="timer";save();render();requestAnimationFrame(()=>document.querySelector(".commonTimerSettings")?.scrollIntoView({block:"start"}));}else start(state.builderName||"Egen økt",state.builderCat||state.builderPickCat,state.builder);};
      ...
      function useWorkoutTemplate(t, edit=false) {
        ...
        state.tab = "builder";
        state.justAdded = "";
        if(edit){state.builderStep="exercises";save();render();return;}
        beginWorkout(t.name,t.cat,state.builder,state.timerChoice);
      }
