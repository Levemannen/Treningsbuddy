const tabs = [["home","Start"],["templates","Øktmaler"],["favoritter","Favoritter"],["push","Push"],["pull","Pull"],["fullbody","Fullbody"],["crosstrening","Cross"],["core","Core"],["kroppsvekt","Kroppsvekt"],["morgen","Morgen"],["yoga","Yoga"],["nodokt","Nødøkt"],["plan","Ukeplan"],["bank","Øvelser"],["builder","Bygger"],["history","Historikk"]];
    ...
    core:{"Planke-varianter":["Planke","Høy planke","Lang planke","RKC-planke","Planke med benløft","Planke med armstrekk","Planke med skuldertouch","Planke til albue (commando plank)","Sideplanke","Sideplanke med benløft","Sideplanke med rotasjon","Copenhagen Plank","Omvendt planke","Planke/Rollout"],"Dead bug / Bird dog":["Dead Bug","Dead bug med armstrekk","Dead bug med benstrekk","Dead bug med arm og ben","Bird Dog","Bird dog med pause","Bird dog crunch"],"Hollow body":["Hollow Hold","Hollow rocks","Hollow tuck hold","Hollow body flutter kicks","Hollow body scissors"],"Crunch-varianter":["Crunch","Reverse Crunch","Bicycle crunch","Toe touch crunch","Heel touch crunch","Oblique crunch","Cross-body crunch","Double crunch"],"Situp-varianter":["Situps","Butterfly situp","Jackknife situp","V-up","Tuck-up","Sprinter situp","Boat Pose"],"Beinhev-varianter":["Leg Raises","Liggende beinhev","Enkeltbeinshev","Saksespark","Flutter kicks","Leg raise hold","Reverse leg raise","Ab Wheel Rollout"],"Skrå magemuskler":["Russian Twist","Russian twist (uten vekt)","Side crunch","Sideplanke med hoftefall","Windshield wipers","Oblique V-up","Plank Knee To Elbow","Pallof Press","Wood Chop"],"L-sit-familien":["Tuck sit","L-sit","Enkeltbeins L-sit","V-sit","Tuck hold"],"Rygg og bakside core":["Superman","Alternating superman","Superman hold","Back Extension","Back extension på gulv","Svømmeren (Swimmer)","Glute Bridge","Enbeins glute bridge","Hip thrust (kroppsvekt)","Hip Extension Hold"],"Dynamiske core-øvelser":["Mountain Climbers","Cross-body mountain climbers","Bear Crawl","Crab walk","Leopard crawl","Army crawl","Inchworm","Plank Walkouts","Plank jacks","Commandos","Shoulder Taps","Stir The Pot"],"Avanserte kroppsvektøvelser":["Dragon flag","Hanging knee raise","Hanging leg raise","Hanging windshield wipers","Toes to bar","Front lever tuck","Front lever raises"]},
        ...
        yoga:{Mobilitet:["Child's Pose","Cat Cow","Downward Dog","Cobra","Low Lunge","Half Split","Pigeon Pose","Seated Forward Fold","Thread The Needle","Supine Twist","Happy Baby","Sphinx","Warrior 1","Warrior 2","Tree Pose","Standing Forward Fold"]}
      };
      const templates = [
        ...
        {id:"crosstrening-a",cat:"crosstrening",name:"Crosstrening A",duration:30,desc:"10 min AMRAP, 10 min intervall, 10 min styrke.",ex:["Pushups","Rows","Air Squats","Burpees","Goblet Hold","Hollow Hold","Plank Walkouts","Dumbbell Deadlift","Mountain Climbers"].map(n=>[n,1,"AMRAP"])},
        {id:"crosstrening-b",cat:"crosstrening",name:"Crosstrening B",desc:"EMOM, sirkel og finisher.",ex:["Push Press","Bent Over Row","Situps","Pushups","Renegade Rows","Burpees"].map(n=>[n,1,"EMOM"])},
        ...
        {id:"cross-interval",cat:"crosstrening",name:"Cross Intervall",duration:24,desc:"40 sek arbeid, 20 sek pause, 4 runder.",ex:["Kettlebell Swing","Pushups","Walking Lunges","Renegade Rows","Burpees","Hollow Hold"].map(n=>[n,1,"40/20"])},
        {id:"yoga-enkel",cat:"yoga",name:"Yoga Enkel",duration:12,desc:"Rolig mobilitet for morgen eller restitusjon.",ex:["Child's Pose","Cat Cow","Downward Dog","Low Lunge","Thread The Needle","Supine Twist"].map(n=>[n,1,"45 sek"])},
      ...
      const workoutTemplates = [
        ...
        {id:"tpl-yoga-calm",cat:"yoga",name:"Rolig yoga",duration:18,level:"Nybegynner",desc:"Rolig mobilitet og pust for hofter, rygg og skuldre.",equipment:"Matte",timerChoice:"interval",work:50,pause:10,rounds:2,ex:[["Child's Pose",1,"50 sek"],["Cat Cow",1,"50 sek"],["Downward Dog",1,"50 sek"],["Low Lunge",1,"50 sek"],["Pigeon Pose",1,"50 sek"],["Supine Twist",1,"50 sek"]]},
        ...
        {id:"tpl-back-mobility",cat:"yoga",name:"Mobilitet for rygg",duration:14,level:"Nybegynner",desc:"Skånsom rutine for rygg, hofter og brystrygg.",equipment:"Matte",timerChoice:"interval",work:45,pause:15,rounds:2,ex:[["Cat Cow",1,"45 sek"],["Thread The Needle",1,"45 sek"],["Sphinx",1,"45 sek"],["Back extension på gulv",1,"45 sek"],["Bird Dog",1,"45 sek"],["Supine Twist",1,"45 sek"]]},
      ...
      const categoryOptions = [["push","Push"],["pull","Pull"],["fullbody","Fullbody"],["crosstrening","Cross"],["kroppsvekt","Kroppsvekt"],["morgen","Morgentrening"],["yoga","Yoga"],["core","Core"]];
      const flowOptions = [["push","Push"],["pull","Pull"],["fullbody","Fullbody"],["crosstrening","Cross"],["kroppsvekt","Kroppsvekt"],["tabata","Tabata"]];
      const intervalCategories = ["crosstrening","kroppsvekt","tabata"];
      ...
      const setFlowCategory = cat => { state.builderPickCat=cat; state.builderCat=cat; state.builderName=`Ny ${flowLabel(cat)}-økt`; state.builder=[]; state.builderSearch=""; state.builderStep="exercises"; state.timerChoice=cat==="tabata"?"tabata":hasIntervalTimer(cat)?"interval":"none"; save(); render(); };
      ...
      const startSavedWorkout = p => { state.timerChoice=p.timerChoice||"none"; state.pauseBetween=p.pauseBetween||0; start(p.name,p.cat||"favoritt",p.ex.map(e=>({...e,setCount:setCountOf(e),targetReps:e.targetReps||e.reps||""}))); };
      ...
      const randomWorkout = () => {
        const pool = poolFor(state.randomCat).filter(Boolean);
        const count = Math.min(Math.max(Number(state.randomCount)||6,5),10);
        const shuffled = [...pool].sort(()=>Math.random()-.5).slice(0,count);
        state.builderName = `Tilfeldig ${categoryOptions.find(x=>x[0]===state.randomCat)?.[1]||"økt"}`;
        state.builderCat = state.randomCat;
        state.builder = shuffled.map(n=>({...entry(n,1,"45 sek"), mode:"intervall", work:45, pause:15, rounds:3}));
        state.tab = "builder";
      ...
      function renderShell() {
        ...
        const recent=state.sessions[0], monthAgo=Date.now()-30*24*60*60*1000;
        document.querySelector("#lastSession").textContent = recent?`Sist: ${recent.name}`:"Klar for første logg";
        ...
        document.querySelector("#startFavorites").onclick=()=>start("Favorittøkt","favoritter",favoriteExercises().map(n=>entry(n,3,"")));
        document.querySelector("#duplicateLast").onclick=()=>{ if(!recent)return; start(`${recent.name} kopi`,recent.cat,recent.ex.map(e=>({...e,setCount:setCountOf(e),targetReps:e.targetReps||e.reps||""}))); };
      }
      ...
      function bindCommon() {
        ...
        document.querySelectorAll("[data-detail]").forEach(b=>b.onclick=()=>{state.detailName=b.dataset.detail; state.detailBack=state.tab; state.tab="detail"; save(); render();});
      ...
      function templateCard(t) { const fav=(state.favoriteTemplates||[]).includes(t.id), stats=templateStats(t); return `<article class="panel grid ${fav?"favoriteCard":""}"><div class="cardTop"><div><h3>${fav?"? ":""}${t.name}</h3><p class="muted">${t.desc||""}</p></div><div class="buttons">${t.duration?`<span class="pill">${t.duration} min</span>`:""}<button class="icon" data-fav-template="${t.id}" title="Favoritt">${fav?"?":"?"}</button></div></div><div class="meta"><span>${stats.count} gjennomføringer</span><span>Første: ${stats.first}</span><span>Siste: ${stats.last}</span><span>Total tid: ${stats.total} min</span><span>Snitt: ${stats.avg||"-"} min</span></div><div class="list">${t.ex.map(x=>`<div class="historyRow clickableRow"><button class="textButton" data-detail="${x[0]}">${x[0]}</button><span class="muted">${x[1]||""} sett ${x[2]||""}</span></div>`).join("")}</div><div class="buttons actionPair"><button class="primary" data-start="${t.id}">Start økt</button><button class="secondary" data-copy="${t.id}">Kopier til bygger</button></div></article>`; }
      ...
      function renderFavoriteWorkouts() {
        ...
        view.innerHTML=`<div class="section"><button class="secondary" id="backHome">Tilbake</button><h2>Favorittøkter</h2></div>${programs.length?`<div class="cards">${programs.map(p=>`<article class="panel grid"><div class="cardTop"><div><h3>${p.name}</h3><p class="muted">${flowLabel(p.cat)} · ${p.ex.length} øvelser</p></div><span class="pill">${p.timerChoice==="tabata"?"Tabata":p.timerChoice==="interval"?"Timer":"Uten timer"}</span></div><div class="list">${p.ex.map(e=>`<div class="historyRow clickableRow"><button class="textButton" data-detail="${e.name}">${e.name}</button><span class="muted">${format(e)}</span></div>`).join("")}</div><div class="buttons actionPair"><button class="primary" data-start-program="${p.id}">Start økt</button><button class="secondary" data-edit-program="${p.id}">Rediger</button><button class="secondary" data-duplicate-program="${p.id}">Dupliser</button><button class="secondary" data-delete-program="${p.id}">Fjern</button></div></article>`).join("")}</div>`:'<p class="empty">Ingen favorittøkter lagret ennå. Lag en ny økt og trykk Lagre som favoritt.</p>'}`;
      ...
      function applyTimerToBuilder() {
        ...
        state.builder=state.builder.map(e=>({...e,mode,work,pause,rounds}));
      }
      ...
      function renderTimerStep() {
        const isTabata=state.builderCat==="tabata";
        const eligible=hasIntervalTimer(state.builderCat);
        if(!eligible) state.timerChoice="none";
        view.innerHTML=`<div class="section"><button class="secondary" id="backExercises">Tilbake</button><h2>${eligible?"Intervaller":"Klar for styrkeøkt"}</h2></div><article class="panel grid"><input id="builderName" value="${state.builderName||"Ny økt"}" placeholder="Navn på økt">${eligible?`<div class="timerChoiceGrid"><button class="chip ${state.timerChoice==="none"?"active":""}" data-timer-choice="none">Ingen timer</button><button class="chip ${state.timerChoice==="interval"?"active":""}" data-timer-choice="interval">Arbeid og hvile</button>${isTabata?`<button class="chip ${state.timerChoice==="tabata"?"active":""}" data-timer-choice="tabata">Tabata 20/10</button>`:""}</div>${state.timerChoice!=="none"?`<div class="timerGrid"><label>Arbeidstid sek<input id="globalWork" inputmode="numeric" value="${state.globalWork||state.builder[0]?.work||20}"></label><label>Hviletid sek<input id="globalPause" inputmode="numeric" value="${state.globalPause||state.builder[0]?.pause||10}"></label><label>Antall runder<input id="globalRounds" inputmode="numeric" value="${state.globalRounds||state.builder[0]?.rounds||8}"></label><label>Pause mellom øvelser<input id="pauseBetween" inputmode="numeric" value="${state.pauseBetween||0}"></label></div>${soundPickerHtml()}`:""}`:'<p class="advice">Styrkeøkten gjennomføres uten intervalltimer. Bytt øvelse når du er klar.</p>'}<div class="list">${state.builder.map((e,i)=>`<div class="historyRow clickableRow"><button class="textButton" data-detail="${e.name}">${e.name}</button><button class="secondary smallText" data-remove="${i}">Fjern</button></div>`).join("")}</div><div class="buttons"><button class="primary" id="startBuilt">Start økt</button><button class="secondary" id="saveProgram">Lagre som favoritt</button></div></article>`;
        document.querySelector("#backExercises").onclick=()=>{state.builderStep="exercises"; save(); render();};
        document.querySelectorAll("[data-timer-choice]").forEach(b=>b.onclick=()=>{state.timerChoice=b.dataset.timerChoice; if(state.timerChoice==="tabata"){state.globalWork=20;state.globalPause=10;state.globalRounds=8;} save(); applyTimerToBuilder(); render();});
        ...
        ["globalWork","globalPause","globalRounds","pauseBetween"].forEach(id=>{const el=document.querySelector("#"+id); if(el)el.oninput=e=>{state[id]=Number(e.target.value)||0; save(); applyTimerToBuilder();};});
      ...
      const intervalRemaining = () => state.intervalRun?.running?Math.max(0,Math.ceil((state.intervalRun.endsAt-Date.now())/1000)):0;
      ...
      const startInterval = i => {armTimerAudio();requestWakeLock();lastCountdownBeep="";const e=state.builder[i]; state.intervalRun={index:i,phase:"work",round:1,running:true,endsAt:Date.now()+(Number(e.work)||20)*1000}; save(); playAppSound("work"); updateCountdowns();};
      ...
      function finishActiveWorkout() {
        if(!state.builder.length)return;
        state.sessions.unshift({date:new Date().toISOString(),name:state.builderName||"Egen økt",cat:state.builderCat||"favoritter",duration:state.startedAt?Math.max(1,Math.round((Date.now()-state.startedAt)/60000)):undefined,ex:state.builder,energy:state.energy,sleep:state.sleep,effort:state.effort,pain:state.pain,comment:state.comment});
      ...
      function renderActiveWorkout() {
        ...
        const edit=document.querySelector("#editWorkout"); if(edit)edit.onclick=()=>{state.builderMode="build";save();render();};
        const close=document.querySelector("#closeFocus"); if(close)close.onclick=()=>{state.builderMode="build";save();render();};
        ...
        const restLeft=Math.max(0,Math.ceil(((state.rest||{})[i]||0-Date.now())/1000));
        const timerHtml=state.timerChoice==="none"?"":`<div class="timerGrid"><label>Timer<select data-i="${i}" data-ek="mode"><option value="styrke" ${e.mode==="styrke"?"selected":""}>Styrke</option><option value="intervall" ${e.mode==="intervall"?"selected":""}>Intervall</option><option value="kontinuerlig" ${e.mode==="kontinuerlig"?"selected":""}>Kontinuerlig</option></select></label><label>Arbeid sek<input inputmode="numeric" data-i="${i}" data-ek="work" value="${e.work||45}"></label><label>Pause sek<input inputmode="numeric" data-i="${i}" data-ek="pause" value="${e.pause||15}"></label><label>Runder<input inputmode="numeric" data-i="${i}" data-ek="rounds" value="${e.rounds||3}"></label></div><div class="buttons">${[60,90,120,180].map(sec=>`<button class="chip ${e.rest===sec?"active":""}" data-rest="${i}:${sec}">${sec}s</button>`).join("")}<button class="primary" data-timer="${i}" data-countdown="${i}">${restLeft?restLeft+"s":"Hvile"}</button><button class="secondary" data-worktimer="${i}">${e.mode==="kontinuerlig"?"Start kontinuerlig":"Start intervall"}</button></div>`;
        return `<div class="builderLine"><div class="builderHead"><div><h3><button class="textButton exerciseName" data-detail="${e.name}">${e.name}</button></h3><p class="muted">Plan: ${format(e)}</p></div><div class="buttons"><button class="secondary smallText" data-detail="${e.name}">Detaljer</button><button class="secondary smallText" data-up="${i}">Opp</button><button class="secondary smallText" data-down="${i}">Ned</button><button class="secondary smallText" data-remove="${i}">Fjern</button></div></div>${timerHtml}<p class="advice">${suggestion(e)}</p><div class="chipRow">${alternatives(e.name).map(n=>`<button class="chip" data-swap="${i}:${n}">Bytt til ${n}</button>`).join("")}</div><textarea data-enote="${i}" placeholder="Kommentar">${e.note||""}</textarea></div>`;
      }
      function bindBuilder() {
        ...
        document.querySelectorAll("[data-builder-cat]").forEach(b=>b.onclick=()=>{state.builderPickCat=b.dataset.builderCat; state.builderCat=b.dataset.builderCat; state.timerChoice=b.dataset.builderCat==="tabata"?"tabata":state.timerChoice; state.builderSearch=""; save(); render();});
        ...
        document.querySelectorAll("[data-load-program]").forEach(b=>b.onclick=()=>{const p=(state.savedPrograms||[]).find(x=>x.id===b.dataset.loadProgram); if(!p)return; state.builderName=p.name; state.builderCat=p.cat; state.builder=p.ex.map(e=>({...e,setCount:setCountOf(e),targetReps:e.targetReps||e.reps||""})); state.startedAt=Date.now(); save(); render();});
        const finish = document.querySelector("#finish");
        if (finish) finish.onclick=()=>{if(!state.builder.length)return; state.sessions.unshift({date:new Date().toISOString(),name:state.builderName||"Egen økt",cat:state.builderCat||"favoritter",duration:state.startedAt?Math.max(1,Math.round((Date.now()-state.startedAt)/60000)):undefined,ex:state.builder,energy:state.energy,sleep:state.sleep,effort:state.effort,pain:state.pain,comment:state.comment}); state.builder=[]; state.rest={}; state.justAdded=""; state.builderMode="build"; state.tab="history"; save(); render();};
